...
class AnchorHelper
{
	public static $extension = 'com_anchor';
	...
	public static function getActions()
	{
		...
		// Get list of actions
		return JHelperContent::getActions('com_anchor');
	}
	...
	public static function getRedirectPluginId()
	{
		...
		$query = $db->getQuery(true)
			...
			->where($db->quoteName('element') . ' = ' . $db->quote('anchor'));
		$db->setQuery($query);
	...
	public static function collectUrlsEnabled()
	{
		...
		if (JPluginHelper::isEnabled('system', 'anchor'))
		{
			$params       = new Registry(JPluginHelper::getPlugin('system', 'anchor')->params);
    ...
    public function get_anchor_arr(){
        $db = JFactory::getDbo();
        $query1 = $db->getQuery(true)
            ->select($db->quoteName('inner_url'))
            ->select($db->quoteName('keyword'))
            ->select($db->quoteName('target_url'))
            ->from('#__anchor');
        $db->setQuery($query1);
