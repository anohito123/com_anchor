...
class JHtmlAnchor
{
	...
	public static function published($value = 0, $i = null, $canChange = true)
	{
		...
		if (null === $i)
		{
			throw new InvalidArgumentException('$i is a required argument in JHtmlAnchor::published');
		}

		// Array of image, task, title, action
		$states = array(
			1  => array('publish', 'links.unpublish', 'JENABLED', '取消发布'),
			0  => array('unpublish', 'links.publish', 'JDISABLED', '发布'),
			2  => array('archive', 'links.unpublish', 'JARCHIVED', 'JUNARCHIVE'),
			-2 => array('trash', 'links.publish', 'JTRASHED', 'COM_REDIRECT1_ENABLE_LINK'),
		);
